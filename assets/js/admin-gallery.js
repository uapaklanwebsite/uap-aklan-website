...
async function initAdminGallery() {
  ...
  async function loadGallery() {
    try {
      ...
      if (container) {
        container.innerHTML = `<div class="col-span-full text-center text-red-400 py-8">Error loading gallery images.</div>`;
      }
    }
  }
  ...
  function renderGallery(items) {
    ...
    if (!items || items.length === 0) {
      container.innerHTML = `<div class="col-span-full text-center text-gray-400 py-8">No gallery images uploaded yet.</div>`;
      return;
    }
    ...
    container.innerHTML = items.map(item => {
      ...
      return `
        <div class="rounded-xl border border-[#D2B866]/20 bg-[#0C2D22] overflow-hidden shadow-lg flex flex-col justify-between group">
          <div class="aspect-video bg-[#143A2E] relative flex items-center justify-center overflow-hidden">
            <img src="${publicUrl}" alt="Gallery Image" class="w-full h-full object-cover" />
          </div>
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        console.log('[Gallery] Saving image_path:', uploadResult.path);

        // 2. Insert image_path into database
        ...
        // Orphan file cleanup if DB insertion failed after upload
        if (newlyUploadedPath) {
          try {
            console.warn('[Admin Gallery] Cleaning up orphaned uploaded image:', newlyUploadedPath);
